...
export default function LoopConsole() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(91,141,239,0.3)', backdropFilter: 'blur(10px)' }}>
      <div className="px-5 py-3 flex items-center gap-3 flex-wrap" style={{ borderBottom: '1px solid rgba(91,141,239,0.15)', background: 'rgba(91,141,239,0.05)' }}>
        <span className="text-[11px] uppercase tracking-wider font-black text-[#5B8DEF]">Compliance Kernel · stressed-bank demo</span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <select value={jur} onChange={e => setJur(e.target.value)}
            className="bg-black/40 text-[11px] text-white rounded px-2 py-1 border border-[rgba(91,141,239,0.3)] outline-none">
          ...
          <button onClick={run} disabled={loading}
            ...
            style={{ background: 'rgba(91,141,239,0.18)', border: '1px solid rgba(91,141,239,0.5)', color: '#5B8DEF' }}>
            {loading ? <><Loader2 className="w-3 h-3 animate-spin" /> Running</> : <><Play className="w-3 h-3" /> Run loop</>}
          </button>
        </div>
      </div>

      {error && <div className="px-5 py-4 text-[12px] text-[#F2566E]">{error}</div>}

      {!data && !loading && (
        <div className="px-5 py-10 text-center text-[12px] text-[rgba(255,255,255,0.5)]">
          Click <strong className="text-[#5B8DEF]">Run loop</strong> — evaluates a deliberately stressed institution
      ...
      {p && (
        <div className="p-5 space-y-4">
          {/* P3 verdict */}
          <Block icon={<Scale className="w-4 h-4" />} color="#5B8DEF" title="Pillar 3 · Code-to-Law verdict">
            <div className="flex items-center gap-3 mb-2">
              {p.p3_code_to_law.compliant
                ? <span className="flex items-center gap-1 text-[#10D982] text-[13px] font-bold"><CheckCircle2 className="w-4 h-4" /> COMPLIANT</span>
                : <span className="flex items-center gap-1 text-[#F2566E] text-[13px] font-bold"><XCircle className="w-4 h-4" /> NON-COMPLIANT</span>}
            ...
            <div className="space-y-1">
              {p.p3_code_to_law.trace.flatMap(t => 'children' in t && (t as { children?: unknown[] }).children ? ((t as { children: typeof t[] }).children) : [t]).slice(0, 8).map((leaf, i) => (
                <div key={i} className="flex items-center gap-2 text-[10px] font-mono">
                  {leaf.passed ? <CheckCircle2 className="w-3 h-3 text-[#10D982] shrink-0" /> : <XCircle className="w-3 h-3 text-[#F2566E] shrink-0" />}
          ...
          {/* P2 red team */}
          <Block icon={<Swords className="w-4 h-4" />} color="#F2566E" title="Pillar 2 · Autonomous red-team">
            ...
            {p.p2_red_team.vectors.slice(0, 3).map((v, i) => (
              <div key={i} className="text-[10px] mb-1.5">
                <span className="font-mono text-[#F2566E]">{v.field}</span>
          ...
          {/* P1 ZK */}
          {p.p1_zk_vault && (
            <Block icon={<Lock className="w-4 h-4" />} color="#10D982" title="Pillar 1 · ZK privacy vault">
              <div className="flex items-center gap-2 text-[12px] mb-2">
                {p.p1_zk_vault.verification.valid
                  ? <span className="text-[#10D982] font-bold flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> proof bundle valid</span>
                  : <span className="text-[#F2566E] font-bold">invalid</span>}
              ...
              {p.p1_zk_vault.verification.checks.map((c, i) => (
                <div key={i} className="flex items-center gap-1.5 text-[10px] font-mono text-[rgba(255,255,255,0.6)]">
                  {c.binding_ok
                    ? <CheckCircle2 className="w-3 h-3 text-[#10D982] shrink-0" />
                    : <XCircle className="w-3 h-3 text-[#F2566E] shrink-0" />}
          ...
          {/* P4 precedent */}
          <Block icon={<Scale className="w-4 h-4" />} color="#F5A524" title="Pillar 4 · Synthetic precedent (Monte Carlo)">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
              <Stat label="P(any action)" value={`${(p.p4_precedent.p_any_action * 100).toFixed(0)}%`} color="#F5A524" />
              <Stat label="Exp. fine" value={`€${p.p4_precedent.expected_fine_eur_m}m`} color="#F5A524" />
              <Stat label="P95 fine" value={`€${p.p4_precedent.worst_case_eur_m}m`} color="#F2566E" />
              <Stat label="Median lag" value={`${p.p4_precedent.median_lag_months}mo`} color="#F5A524" />
            </div>
          </Block>

          {/* P6 twin */}
          <Block icon={<Brain className="w-4 h-4" />} color="#5B8DEF" title="Pillar 6 · Regulatory twin (institutional)">
            <div className="flex items-center gap-3 mb-1.5">
              <span className="text-[13px] font-bold text-white">{p.p6_regulatory_twin.supervisor}</span>
              <span className="text-[11px] font-mono" style={{ color: p.p6_regulatory_twin.enforcement_risk >= 65 ? '#F2566E' : p.p6_regulatory_twin.enforcement_risk >= 45 ? '#F5A524' : '#10D982' }}>
                risk {p.p6_regulatory_twin.enforcement_risk}/100
              </span>
              <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded" style={{ background: 'rgba(91,141,239,0.12)', border: '1px solid rgba(91,141,239,0.3)', color: '#5B8DEF' }}>
          ...
          {/* P5 topology */}
          <Block icon={<Network className="w-4 h-4" />} color="#5B8DEF" title="Pillar 5 · Topological law mapping">
            ...
            {p.p5_topology.loopholes.slice(0, 3).map((l, i) => (
              <div key={i} className="flex items-start gap-2 text-[10px] mb-1">
                <AlertTriangle className="w-3 h-3 text-[#5B8DEF] shrink-0 mt-0.5" />
                <span className="text-[rgba(255,255,255,0.6)]"><span className="text-[#5B8DEF] font-bold">[{l.severity}]</span> {l.description}</span>
              </div>
            ))}
          </Block>

          {/* P7 kinetic */}
          <Block icon={<Zap className="w-4 h-4" />} color="#F5A524" title="Pillar 7 · Kinetic compliance (signed intents)">
            {p.p7_kinetic.map((intent, i) => (
              <div key={i} className="rounded-lg p-2.5 mb-1.5" style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(245,165,36,0.2)' }}>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-[10px] font-mono text-[#F5A524] font-bold">{intent.id}</span>
                  ...
                  <span className="text-[8px] uppercase tracking-wider px-1.5 py-0.5 rounded ml-auto" style={{
                    background: intent.urgency === 'immediate' ? 'rgba(242,86,110,0.15)' : 'rgba(245,165,36,0.12)',
                    color: intent.urgency === 'immediate' ? '#F2566E' : '#F5A524',
