...
function Verdict({ ok, okLabel, badLabel }: { ok: boolean; okLabel: string; badLabel: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-[12px] font-bold" style={{ color: ok ? '#10D982' : '#F2566E' }}>
...
function SubstancePanel() {
  ...
  return (
    <div className="rounded-xl p-4" style={card('#5B8DEF')}>
      <Head icon={<MapPin className="w-4 h-4" />} color="#5B8DEF" n={1} title="Substance Audit · geofence (CSSF 24/856)" />
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {presets.map((p, i) => (
          <button key={p.label} onClick={() => setSel(i)}
            ...
            style={{ background: i === sel ? 'rgba(91,141,239,0.25)' : 'rgba(255,255,255,0.04)', border: `1px solid ${i === sel ? 'rgba(91,141,239,0.5)' : 'rgba(255,255,255,0.1)'}`, color: i === sel ? '#fff' : 'rgba(255,255,255,0.6)' }}>
            {p.label}
          </button>
        ))}
      </div>
      ...
      <WhyChip cite={SUBSTANCE_CITATION} accent="#5B8DEF" />
    </div>
  )
}
...
function ReconPanel() {
  ...
  return (
    <div className="rounded-xl p-4" style={card('#5B8DEF')}>
      <Head icon={<GitCompare className="w-4 h-4" />} color="#5B8DEF" n={2} title="Cross-Departmental Discrepancy Engine" />
      <div className="flex gap-1.5 mb-3">
        {(['breach', 'clean'] as const).map(s => (
          <button key={s} onClick={() => setScenario(s)} className="text-[10px] px-2 py-1 rounded uppercase tracking-wider font-bold"
            style={{ background: scenario === s ? 'rgba(91,141,239,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${scenario === s ? 'rgba(91,141,239,0.5)' : 'rgba(255,255,255,0.1)'}`, color: scenario === s ? '#5B8DEF' : 'rgba(255,255,255,0.5)' }}>
      ...
      <div className="space-y-1">
        {res.discrepancies.map((d: Discrepancy, i) => (
          <div key={i} className="text-[10px] leading-snug" style={{ color: d.severity === 'critical' ? '#F2566E' : '#F5A524' }}>
            <span className="font-mono font-bold">[{d.severity}] {d.code}</span> <span className="text-[rgba(255,255,255,0.6)]">{d.detail}</span>
            <WhyChip cite={RECON_CITATIONS[d.code]} accent="#5B8DEF" />
          </div>
        ))}
...
function AifmdPanel() {
  ...
  return (
    <div className="rounded-xl p-4" style={card('#F5A524')}>
      <Head icon={<TrendingUp className="w-4 h-4" />} color="#F5A524" n={3} title="AIFMD II · pre-trade /simulate" />
        ...
        <div className="flex gap-1.5">
          {(['open_ended', 'closed_ended'] as const).map(s => (
            <button key={s} onClick={() => setStructure(s)} className="text-[10px] px-2 py-1 rounded"
              style={{ background: structure === s ? 'rgba(245,165,36,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${structure === s ? 'rgba(245,165,36,0.5)' : 'rgba(255,255,255,0.1)'}`, color: structure === s ? '#F5A524' : 'rgba(255,255,255,0.5)' }}>
      ...
      <div className="space-y-1">
        {res.checks.map((c: RuleCheck) => (
          <div key={c.rule} className="flex items-start gap-1.5 text-[10px]">
            {c.passed ? <CheckCircle2 className="w-3 h-3 text-[#10D982] shrink-0 mt-0.5" /> : <XCircle className="w-3 h-3 text-[#F2566E] shrink-0 mt-0.5" />}
            <span className="min-w-0">
              ...
              <WhyChip cite={AIFMD_CITATIONS[c.rule]} accent="#F5A524" />
            </span>
          </div>
        ))}
      </div>
...
function EidPanel() {
  ...
  return (
    <div className="rounded-xl p-4" style={card('#5B8DEF')}>
      <Head icon={<FileCheck2 className="w-4 h-4" />} color="#5B8DEF" n={4} title="CSSF e-Identification · pre-flight" />
      <div className="flex gap-1.5 mb-3">
        {[['incomplete package', false], ['complete package', true]].map(([label, v]) => (
          <button key={String(v)} onClick={() => setValid(v as boolean)} className="text-[10px] px-2 py-1 rounded"
            style={{ background: valid === v ? 'rgba(91,141,239,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${valid === v ? 'rgba(91,141,239,0.5)' : 'rgba(255,255,255,0.1)'}`, color: valid === v ? '#5B8DEF' : 'rgba(255,255,255,0.5)' }}>
            {label as string}
          </button>
        ))}
      </div>
        ...
        {res.findings.map((f: EidFinding) => (
          <div key={f.field} className="text-[10px]">
            <div className="flex items-center gap-1.5">
              {f.ok ? <CheckCircle2 className="w-3 h-3 text-[#10D982] shrink-0" /> : <XCircle className="w-3 h-3 text-[#F2566E] shrink-0" />}
              <span className="font-mono text-[rgba(255,255,255,0.7)]">{f.field}</span>
              <span className="text-[rgba(255,255,255,0.45)]">— {f.message}</span>
            </div>
            <div className="ml-[18px]"><WhyChip cite={EID_CITATIONS[f.field]} accent="#5B8DEF" /></div>
          </div>
        ))}
      </div>
    </div>
...
function DelegationPanel() {
  ...
  const color = res.score >= 70 ? '#10D982' : res.score >= 50 ? '#F5A524' : '#F2566E'
  return (
    <div className="rounded-xl p-4" style={card('#F2566E')}>
      <Head icon={<ShieldAlert className="w-4 h-4" />} color="#F2566E" n={5} title="Delegation Oversight (CSSF 18/698)" />
      ...
      <WhyChip cite={DELEGATION_CITATION} accent="#F2566E" />
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl p-4" style={card('#10D982')}>
          <Head icon={<ShieldAlert className="w-4 h-4" />} color="#10D982" n={0} title="Tamper-evident audit ledger" />
          ...
          <button onClick={runChainProof} disabled={chainBusy}
            ...
            style={{ background: 'rgba(16,217,130,0.15)', border: '1px solid rgba(16,217,130,0.5)', color: '#10D982' }}>
            <Play className="w-3 h-3" /> {chainBusy ? 'running' : 'run tamper proof'}
          </button>
          {chainState && (
            <div className="text-[10px] font-mono leading-relaxed space-y-1">
              <div className="text-[rgba(255,255,255,0.5)]">link[0] entryHash: <span className="text-[#10D982]">{chainState.hash0.slice(0, 24)}…</span></div>
              <div className="flex items-center gap-1.5" style={{ color: '#10D982' }}>
              ...
              <div className="flex items-center gap-1.5" style={{ color: '#F2566E' }}>
