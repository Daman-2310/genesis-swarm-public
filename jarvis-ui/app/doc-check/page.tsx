...
const StatusIcon = ({ s }: { s: string }) => s === 'found' ? <CheckCircle className="w-3.5 h-3.5 text-[#10D982]" /> : <XCircle className="w-3.5 h-3.5 text-[#F2566E]" />
...
export default function DocCheckPage() {
  ...
  const inp = 'bg-[rgba(16,217,130,0.03)] border border-[rgba(16,217,130,0.18)] rounded px-3 py-2 text-[#10D982] text-xs font-mono placeholder-[rgba(16,217,130,0.25)] focus:outline-none focus:border-[rgba(16,217,130,0.5)] transition-all'
    ...
    <div className="min-h-screen bg-[#050508] text-[#10D982] font-mono overflow-x-hidden">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b border-[rgba(16,217,130,0.12)]" style={{ background: 'rgba(5,5,8,0.97)', backdropFilter: 'blur(8px)' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[rgba(16,217,130,0.4)] text-[10px] tracking-widest hidden sm:block">// Regulatory Document Checker</span>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        {!result ? (
          <div className="space-y-6">
            <div>
              <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.5)] mb-2">DORA · SFDR · AIFMD II · UCITS</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1.5">Regulation</label>
                <select className={inp + ' w-full cursor-pointer'} value={regulation} onChange={e => setRegulation(e.target.value)}>
                  {REGULATIONS.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1.5">Fund type</label>
              ...
              <div>
                <label className="block text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1.5">Fund name (optional)</label>
                <input className={inp + ' w-full'} placeholder="Acme Capital AIFM" value={fundName} onChange={e => setFundName(e.target.value)} />
              </div>
            </div>

            <div>
              <label className="block text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1.5">Document text</label>
              <textarea
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[7px] text-[rgba(16,217,130,0.25)]">Minimum 50 characters</span>
                <span className="text-[7px] text-[rgba(16,217,130,0.35)]">{text.split(/\s+/).filter(Boolean).length} words</span>
              </div>
            </div>
          ...
          <div className="space-y-6">
            {/* Score header */}
            <div className="p-6 rounded" style={{ background: 'rgba(16,217,130,0.02)', border: `2px solid ${gradeColor(result.coverage_grade)}33` }}>
              <div className="flex items-start justify-between gap-6 mb-4">
                <div className="flex-1">
                  <div className="text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1">{result.fund_name} · {result.regulation} · {result.word_count.toLocaleString()} words</div>
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                {[
                  { label: 'Checked', val: result.requirements_checked, color: 'rgba(16,217,130,0.6)' },
                  { label: 'Found', val: result.found_count, color: '#10D982' },
                  { label: 'Missing', val: result.missing_count, color: '#F2566E' },
                ].map(({ label, val, color }) => (
            ...
            {result.top_recommendations.length > 0 && (
              <div className="p-4 rounded space-y-2" style={{ background: 'rgba(242,86,110,0.04)', border: '1px solid rgba(242,86,110,0.2)' }}>
                <div className="flex items-center gap-2 mb-2">
                  <AlertCircle className="w-4 h-4 text-[#F2566E]" />
                  <span className="text-[9px] uppercase tracking-widest text-[#F2566E] font-bold">Top recommendations</span>
                </div>
                {result.top_recommendations.map((r, i) => (
                  <div key={i} className="text-[9px] text-[rgba(255,255,255,0.55)] leading-relaxed flex gap-2">
                    <span className="text-[rgba(242,86,110,0.6)] shrink-0">{i+1}.</span>{r}
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Detailed analysis */}
            <div>
              <div className="text-[9px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-3">Detailed requirement analysis</div>
              <div className="space-y-1.5">
                {result.analysis.map((a, i) => (
                  <div key={i} className="rounded" style={{ background: a.status === 'found' ? 'rgba(16,217,130,0.02)' : 'rgba(242,86,110,0.03)', border: `1px solid ${a.status === 'found' ? 'rgba(16,217,130,0.15)' : 'rgba(242,86,110,0.2)'}` }}>
                    <button className="w-full flex items-center gap-3 p-3 text-left" onClick={() => toggleExpand(i)}>
                      ...
                      <span className="text-[8px] uppercase tracking-wider px-2 py-0.5 rounded font-bold" style={{ background: `rgba(16,217,130,0.06)`, color: 'rgba(16,217,130,0.5)' }}>{a.regulation}</span>
                      {expanded.has(i) ? <ChevronDown className="w-3 h-3 text-[rgba(16,217,130,0.4)]" /> : <ChevronRight className="w-3 h-3 text-[rgba(16,217,130,0.4)]" />}
                    </button>
                    {expanded.has(i) && (
                      <div className="px-3 pb-3 space-y-1">
                        {a.matched_keyword && <div className="text-[8px] text-[#10D982]">Matched: <span className="font-bold">"{a.matched_keyword}"</span></div>}
            ...
            <button onClick={() => setResult(null)} className="px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider" style={{ background: 'rgba(16,217,130,0.06)', border: '1px solid rgba(16,217,130,0.3)', color: '#10D982' }}>← Check another document</button>
          </div>
        )}
      </div>
    </div>
