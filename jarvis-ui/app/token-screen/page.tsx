...
const severityColor: Record<Finding['severity'], string> = {
  critical: '#F2566E', warning: '#F5A524', info: '#5B8DEF', pass: '#10D982',
}
...
export default function TokenScreenPage() {
  ...
  const scoreColor = result ? (result.complianceScore >= 80 ? '#10D982' : result.complianceScore >= 60 ? '#F5A524' : '#F2566E') : '#10D982'
    ...
    <div className="min-h-screen text-white" style={{ background: 'radial-gradient(ellipse at top, #0a0a1a 0%, #050508 50%, #000 100%)' }}>
      <header className="sticky top-0 z-30 border-b border-[rgba(255,255,255,0.06)] px-6 py-3 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          ...
          <Coins className="w-4 h-4 text-[#F5A524]" />
          <span className="text-sm font-bold tracking-[0.18em] text-[#F5A524]">RWA TOKEN COMPLIANCE</span>
        </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5"
            ...
            <span className="w-1.5 h-1.5 rounded-full bg-[#F5A524]"
              style={{ animation: 'pulse 1.2s ease-in-out infinite', boxShadow: '0 0 8px #F5A524' }} />
            <span className="text-[10px] font-black tracking-[0.25em] uppercase text-[#F5A524]">LUXEMBOURG = #1 RWA HUB</span>
          </div>
          <h1 className="font-black tracking-tight mb-3" style={{ fontSize: 'clamp(2.5rem, 5vw, 4.5rem)', lineHeight: 1 }}>
            ...
            <br />
            <span style={{ background: 'linear-gradient(90deg, #F5A524 0%, #ff6b3d 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              tokenized assets.
            </span>
          </h1>
        ...
        <div className="rounded-2xl p-6 mb-6"
          style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(245,165,36,0.2)' }}>
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <input value={address} onChange={e => setAddress(e.target.value)}
              ...
              style={{ border: '1px solid rgba(245,165,36,0.3)' }} />
            ...
            <button onClick={screen} disabled={busy || !address.trim()}
              ...
              style={{
                background: 'linear-gradient(135deg, #F5A524 0%, #ff6b3d 100%)',
                color: '#000',
                boxShadow: '0 0 16px rgba(245,165,36,0.4)',
              }}>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {EXAMPLES.map(ex => (
              <button key={ex.label} onClick={() => { setAddress(ex.addr); setChain(ex.chain) }}
                ...
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(245,165,36,0.25)', color: 'rgba(245,165,36,0.85)' }}>
                {ex.label}
              </button>
            ))}
          </div>
          ...
          {error && (
            <div className="mt-3 flex items-center gap-2 p-3 rounded text-[11px]"
              style={{ background: 'rgba(242,86,110,0.06)', border: '1px solid rgba(242,86,110,0.3)', color: '#F2566E' }}>
        ...
        {result && (
          <div className="space-y-4">
            ...
            <div className="rounded-2xl p-6"
              style={{ background: 'linear-gradient(135deg, rgba(245,165,36,0.04) 0%, rgba(255,107,61,0.03) 100%)', border: '1px solid rgba(245,165,36,0.3)' }}>
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-mono text-[11px] text-[rgba(245,165,36,0.85)] truncate">{result.address}</div>
                  <div className="flex items-center gap-3 mt-2 text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.5)]">
                    ...
                    <span className="text-[#F5A524] font-bold">{result.detectedStandard}</span>
                    {result.isPaused === true && <span className="text-[#F2566E] font-bold">⏸ PAUSED</span>}
                    {result.explorer && (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.regulatoryFlags.map((f, i) => (
                    <span key={i} className="text-[9px] px-2 py-0.5 rounded-full uppercase tracking-wider"
                      style={{ background: 'rgba(245,165,36,0.08)', border: '1px solid rgba(245,165,36,0.35)', color: '#F5A524' }}>
                      {f}
                    </span>
                  ))}
                </div>
            ...
            <div className="rounded-2xl p-5"
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] uppercase tracking-[0.2em] font-black text-[#10D982]">RECOMMENDATION</span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-8">
            {[
              ...
              <div key={s.t} className="rounded-lg p-4"
                ...
                <CheckCircle2 className="w-4 h-4 text-[#F5A524] mb-2" />
