...
const riskColor = (r: string) => r === 'high' ? '#F2566E' : r === 'review' ? '#F5A524' : '#10D982'
const riskBg = (r: string) => r === 'high' ? 'rgba(242,86,110,0.08)' : r === 'review' ? 'rgba(245,165,36,0.08)' : 'rgba(16,217,130,0.05)'
const riskBorder = (r: string) => r === 'high' ? 'rgba(242,86,110,0.35)' : r === 'review' ? 'rgba(245,165,36,0.35)' : 'rgba(16,217,130,0.25)'
...
export default function ScreeningPage() {
  ...
  const inp = 'bg-[rgba(16,217,130,0.03)] border border-[rgba(16,217,130,0.18)] rounded px-2.5 py-2 text-[#10D982] text-xs font-mono placeholder-[rgba(16,217,130,0.25)] focus:outline-none focus:border-[rgba(16,217,130,0.5)] transition-all w-full'
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        {!result ? (
          <div className="space-y-6">
            <div>
              <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.5)] mb-2">AML · 4AMLD · FATF Compliance</div>
            ...
            {/* Screening lists */}
            <div>
              <div className="text-[9px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-3">Screening lists</div>
              <div className="flex flex-wrap gap-2">
                {LISTS.map(l => (
                  <button key={l} onClick={() => toggleList(l)} className="flex items-center gap-2 px-3 py-1.5 rounded text-[10px] uppercase tracking-wider font-bold transition-all"
                    style={{ background: selectedLists.includes(l) ? 'rgba(16,217,130,0.12)' : 'rgba(16,217,130,0.02)', border: `1px solid ${selectedLists.includes(l) ? 'rgba(16,217,130,0.5)' : 'rgba(16,217,130,0.18)'}`, color: selectedLists.includes(l) ? '#10D982' : 'rgba(16,217,130,0.4)' }}>
            ...
            <div className="space-y-3">
              ...
              {entities.map((e, i) => (
                <div key={e.id} className="grid grid-cols-12 gap-2 p-3 rounded" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.12)' }}>
                  ...
                  <div className="col-span-1 flex items-center justify-center">
                    {entities.length > 1 && <button onClick={() => remove(e.id)} className="text-[rgba(242,86,110,0.4)] hover:text-[#F2566E] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>}
                  </div>
                </div>
              ))}
              <button onClick={add} className="flex items-center gap-2 px-4 py-2 rounded text-[10px] uppercase tracking-wider transition-all" style={{ background: 'rgba(16,217,130,0.04)', border: '1px dashed rgba(16,217,130,0.25)', color: 'rgba(16,217,130,0.6)' }}>
          ...
          <div className="space-y-6">
            {/* Summary bar */}
            <div className="p-5 rounded grid grid-cols-2 sm:grid-cols-4 gap-4" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.18)' }}>
              <div><div className="text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.4)] mb-1">Screening ID</div><div className="text-xs font-bold text-white">{result.screening_id}</div></div>
              <div><div className="text-[8px] uppercase tracking-widest text-[rgba(242,86,110,0.6)] mb-1">High risk</div><div className="text-2xl font-black text-[#F2566E]">{result.high_count}</div></div>
              <div><div className="text-[8px] uppercase tracking-widest text-[rgba(245,165,36,0.6)] mb-1">Review</div><div className="text-2xl font-black text-[#F5A524]">{result.review_count}</div></div>
              <div><div className="text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.6)] mb-1">Clear</div><div className="text-2xl font-black text-[#10D982]">{result.clear_count}</div></div>
            </div>
                    ...
                    <div className="space-y-1.5">
                      {r.hits.map((h, j) => (
                        <div key={j} className="flex items-start gap-2 text-[8px] text-[rgba(255,255,255,0.5)]">
                          <span className="text-[rgba(245,165,36,0.8)] shrink-0 mt-0.5">→</span>
            ...
            <div className="text-[7px] text-[rgba(255,255,255,0.2)] leading-relaxed p-3 rounded" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <span className="text-[rgba(16,217,130,0.4)] font-bold">Methodology:</span> {result.methodology}<br />
              <span className="text-[rgba(16,217,130,0.4)] font-bold">Regulatory basis:</span> {result.regulatory_basis}
            </div>

            <button onClick={() => setResult(null)} className="px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider" style={{ background: 'rgba(16,217,130,0.06)', border: '1px solid rgba(16,217,130,0.3)', color: '#10D982' }}>← New screening</button>
          </div>
        )}
      </div>
    </div>
