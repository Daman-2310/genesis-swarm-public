...
const statusColor = (s: string) => s === 'compliant' ? '#10D982' : s === 'partial' ? '#F5A524' : '#F2566E'
...
export default function PortfolioPage() {
  ...
  const inp = 'bg-[rgba(16,217,130,0.03)] border border-[rgba(16,217,130,0.2)] rounded px-2.5 py-2 text-[#10D982] text-xs font-mono placeholder-[rgba(16,217,130,0.25)] focus:outline-none focus:border-[rgba(16,217,130,0.5)] transition-all'
    ...
    <div className="min-h-screen bg-[#050508] text-[#10D982] font-mono overflow-x-hidden">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b border-[rgba(16,217,130,0.12)]" style={{ background: 'rgba(5,5,8,0.97)', backdropFilter: 'blur(8px)' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[rgba(16,217,130,0.4)] text-[10px] tracking-widest hidden sm:block">// Multi-Fund Portfolio Dashboard</span>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        {!result && (
          <div className="space-y-6">
            <div>
              <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.5)] mb-2">Enterprise · Multi-Fund View</div>
            ...
            <div className="space-y-3">
              <div className="hidden sm:grid grid-cols-12 gap-2 px-3 text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.35)]">
              ...
              {funds.map((f, i) => (
                <div key={f.id} className="grid grid-cols-12 gap-2 p-3 rounded" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.12)' }}>
                  ...
                  <div className="col-span-1 flex items-center justify-center">
                    {funds.length > 1 && <button onClick={() => removeFund(f.id)} className="text-[rgba(242,86,110,0.5)] hover:text-[#F2566E] transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>}
                  </div>
                </div>
              ))}
              <button onClick={addFund} className="flex items-center gap-2 px-4 py-2 rounded text-[10px] uppercase tracking-wider transition-all" style={{ background: 'rgba(16,217,130,0.04)', border: '1px dashed rgba(16,217,130,0.25)', color: 'rgba(16,217,130,0.6)' }}>
            ...
            <button onClick={assess} disabled={!valid || loading} className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3.5 rounded font-bold text-sm uppercase tracking-wider transition-all disabled:opacity-50"
              style={{ background: valid ? 'rgba(16,217,130,0.12)' : 'rgba(16,217,130,0.04)', border: `1px solid ${valid ? 'rgba(16,217,130,0.6)' : 'rgba(16,217,130,0.2)'}`, color: '#10D982', boxShadow: valid ? '0 0 20px rgba(16,217,130,0.15)' : 'none' }}>
        ...
        {result && (
          <div className="space-y-6">
            {/* Portfolio header */}
            <div className="p-6 rounded" style={{ background: 'rgba(16,217,130,0.03)', border: `2px solid ${gradeColor(result.portfolio_grade)}33`, boxShadow: `0 0 40px ${gradeColor(result.portfolio_grade)}11` }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.4)] mb-1">Portfolio Assessment · {result.total_funds} funds</div>
              ...
              {result.critical_funds.length > 0 && (
                <div className="flex items-center gap-2 p-3 rounded" style={{ background: 'rgba(242,86,110,0.08)', border: '1px solid rgba(242,86,110,0.25)' }}>
                  <AlertTriangle className="w-4 h-4 text-[#F2566E] shrink-0" />
                  <span className="text-[9px] text-[rgba(255,255,255,0.6)]">Critical attention: <span className="text-[#F2566E] font-bold">{result.critical_funds.join(', ')}</span></span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {result.funds.map(fund => (
                <div key={fund.fund_name} className="p-4 rounded space-y-3" style={{ background: 'rgba(16,217,130,0.02)', border: `1px solid ${gradeColor(fund.grade)}33` }}>
                  ...
                  {fund.critical_count > 0 && (
                    <div className="text-[8px] text-[rgba(255,255,255,0.5)] leading-relaxed">
                      <span className="text-[#F2566E] font-bold">{fund.critical_count} critical</span>{fund.high_count > 0 && <span>, <span className="text-[#F5A524]">{fund.high_count} high</span></span>} gaps · {fund.top_gap}
                    </div>
                  )}
                  <div className="flex gap-2 pt-1">
                    <a href="/onboard" className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-[8px] uppercase tracking-wider transition-all" style={{ background: 'rgba(16,217,130,0.06)', border: '1px solid rgba(16,217,130,0.25)', color: '#10D982' }}>
                      <ExternalLink className="w-2.5 h-2.5" /> Full assessment
                    </a>
                    <a href="/board-report" className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-[8px] uppercase tracking-wider transition-all" style={{ background: 'rgba(91,141,239,0.06)', border: '1px solid rgba(91,141,239,0.25)', color: '#5B8DEF' }}>
                      Board report
                    </a>
                  </div>
                </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setResult(null)} className="px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider" style={{ background: 'rgba(16,217,130,0.06)', border: '1px solid rgba(16,217,130,0.3)', color: '#10D982' }}>← Edit funds</button>
              <a href="/trial" className="flex items-center gap-2 px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider" style={{ background: 'rgba(16,217,130,0.1)', border: '1px solid rgba(16,217,130,0.5)', color: '#10D982' }}><Zap className="w-4 h-4" /> Start pilot</a>
            </div>
          </div>
        )}
      </div>
