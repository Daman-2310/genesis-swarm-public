...
export async function GET(req: NextRequest) {
  ...
  // Phase 3 — Continuous-compliance monitoring sweep (re-evaluate monitored funds)
  let monitorResult: Awaited<ReturnType<typeof runMonitorSweep>> = { configured: false, checked: 0, alerted: 0 }
  try {
    monitorResult = await runMonitorSweep()
  } catch (e) {
    monitorResult = { configured: true, checked: 0, alerted: 0, error: String(e).slice(0, 200) }
  }
