...
function Toggle({ value, onChange, label }: { value: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button type="button" onClick={() => onChange(!value)}
      className={`flex items-center gap-2 px-3 py-2 rounded border text-xs font-mono transition-colors ${value ? 'border-[rgba(16,217,130,0.5)] bg-[rgba(16,217,130,0.08)] text-[#10D982]' : 'border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.4)] hover:border-[rgba(255,255,255,0.2)]'}`}>
      <span className={`w-3 h-3 rounded-full border ${value ? 'bg-[#10D982] border-[#10D982]' : 'border-[rgba(255,255,255,0.3)]'}`} />
      {label}
    </button>
  )
}
  ...
  return (
    <div>
      ...
      <select value={value} onChange={e => onChange(e.target.value)}
        className="w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.12)] rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-[rgba(91,141,239,0.5)]">
...
function StepDot({ n, active, done }: { n: number; active: boolean; done: boolean }) {
  return (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border transition-all ${done ? 'bg-[#10D982] border-[#10D982] text-black' : active ? 'border-[rgba(91,141,239,0.8)] text-[#5B8DEF] bg-[rgba(91,141,239,0.1)]' : 'border-[rgba(255,255,255,0.15)] text-[rgba(255,255,255,0.3)]'}`}>
...
function GapCard({ gap, expanded, onToggle }: { gap: GapItem; expanded: boolean; onToggle: () => void }) {
  const pc = { critical: '#F2566E', high: '#ff6b35', medium: '#F5A524', low: '#5B8DEF' }[gap.priority]
  const sc = { missing: '#F2566E', partial: '#F5A524', met: '#10D982' }[gap.status]
  return (
    <div className="border border-[rgba(255,255,255,0.07)] rounded bg-[rgba(255,255,255,0.02)]">
      ...
      {expanded && (
        <div className="px-3 pb-3 border-t border-[rgba(255,255,255,0.05)]">
          <p className="text-[11px] text-[#5B8DEF] mt-2">Action: {gap.action}</p>
...
export default function OnboardPage() {
  ...
  const gradeColor = (g: string) => ({ A: '#10D982', B: '#5B8DEF', C: '#F5A524', D: '#ff6b35', F: '#F2566E' }[g] ?? '#fff')
    ...
    <div className="min-h-screen bg-[#050a0e] text-white font-mono">
      <div className="border-b border-[rgba(16,217,130,0.1)] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <a href="/dashboard" className="text-[10px] text-[rgba(255,255,255,0.4)] hover:text-white flex items-center gap-1"><ArrowLeft className="w-3 h-3" /> Dashboard</a>
          <div className="w-px h-4 bg-[rgba(255,255,255,0.1)]" />
          <span className="text-sm font-bold tracking-widest text-[#10D982]">COMPLIANCE ONBOARDING</span>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-center gap-2 mb-8 flex-wrap">
            {['Fund Identity', 'Strategy', 'Sustainability', 'Compliance'].map((label, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className={`text-[10px] hidden sm:block ${step === i+1 ? 'text-[#5B8DEF]' : step > i+1 ? 'text-[#10D982]' : 'text-[rgba(255,255,255,0.2)]'}`}>{label}</span>
                {i < 3 && <div className={`w-6 h-px ${step > i+1 ? 'bg-[#10D982]' : 'bg-[rgba(255,255,255,0.1)]'}`} />}
              </div>
            ))}
          </div>
        )}
        ...
        {step === 5 && result && (
          <div className="space-y-6">
            <div className="border border-[rgba(16,217,130,0.2)] bg-[rgba(16,217,130,0.02)] rounded-lg p-5">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                {[{label:'Critical',value:result.critical_count,color:'#F2566E'},{label:'High',value:result.high_count,color:'#ff6b35'},{label:'Total Gaps',value:result.total_gaps,color:'#F5A524'}].map(s => (
            ...
            {result.immediate_actions.length > 0 && (
              <div>
                <h3 className="text-xs font-bold text-[#F2566E] uppercase tracking-wider mb-2 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Immediate Actions</h3>
                <div className="space-y-2">
                  {result.immediate_actions.map((g, i) => (
                    <div key={i} className="p-3 border border-[rgba(242,86,110,0.25)] bg-[rgba(242,86,110,0.04)] rounded text-xs">
                ...
                <div className="flex gap-1 flex-wrap">
                  {frameworks.map(fw => (
                    <button key={fw} onClick={() => setFilterFw(fw)}
                      className={`text-[9px] px-2 py-0.5 rounded border transition-colors ${filterFw === fw ? 'border-[rgba(91,141,239,0.6)] text-[#5B8DEF] bg-[rgba(91,141,239,0.08)]' : 'border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.35)]'}`}>
                      {fw === 'all' ? 'All' : fw}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <button onClick={downloadReport} className="flex items-center gap-2 px-4 py-2.5 bg-[rgba(16,217,130,0.08)] border border-[rgba(16,217,130,0.4)] text-[#10D982] rounded text-sm font-bold hover:bg-[rgba(16,217,130,0.12)] transition-colors">
                <Download className="w-4 h-4" /> Download Report
              </button>
              <a href="/sfdr" className="flex items-center gap-2 px-4 py-2.5 border border-[rgba(91,141,239,0.3)] text-[#5B8DEF] rounded text-sm hover:bg-[rgba(91,141,239,0.08)] transition-colors">Generate SFDR →</a>
        ...
        {error && (
          <div className="flex items-center gap-2 p-3 border border-[rgba(242,86,110,0.3)] bg-[rgba(242,86,110,0.05)] rounded text-sm text-[#F2566E] mt-4">
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < 4 ? (
              <button onClick={() => setStep(s => s + 1)} disabled={step === 1 && !profile.fund_name.trim()}
                className="flex items-center gap-2 px-5 py-2 bg-[rgba(91,141,239,0.1)] border border-[rgba(91,141,239,0.5)] text-[#5B8DEF] rounded text-sm font-bold hover:bg-[rgba(91,141,239,0.15)] disabled:opacity-40 transition-colors">
                Next <ArrowRight className="w-4 h-4" />
              </button>
            ) : (
              <button onClick={runAssessment} disabled={loading}
                className="flex items-center gap-2 px-5 py-2 bg-[rgba(16,217,130,0.1)] border border-[rgba(16,217,130,0.5)] text-[#10D982] rounded text-sm font-bold hover:bg-[rgba(16,217,130,0.15)] disabled:opacity-40 transition-colors">
