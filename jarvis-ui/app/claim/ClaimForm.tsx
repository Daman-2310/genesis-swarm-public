...
export default function ClaimForm() {
    ...
    return (
      <div className="rounded-2xl p-5 space-y-4"
        ...
        <div className="flex items-start gap-3">
          <CheckCircle2 className="w-6 h-6 text-[#10D982] shrink-0 mt-1" />
          <div>
            <div className="text-[14px] uppercase tracking-[0.18em] font-black text-[#10D982] mb-1">Application received</div>
            ...
            <div className="text-[10px] text-[rgba(255,255,255,0.5)] mt-3 font-mono">
              Application ID: <span className="text-[#10D982]">{success.application_id}</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="rounded-2xl p-5 space-y-4"
      style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(16,217,130,0.25)', backdropFilter: 'blur(10px)' }}>

      {/* Tier selection */}
      <div>
        <label className="block text-[10px] uppercase tracking-wider font-bold text-[#10D982] mb-2">Tier</label>
          ...
          {(['standard', 'premium'] as const).map(t => (
            <button key={t} onClick={() => setForm(f => ({ ...f, tier: t }))}
              ...
              style={{
                ...
                border: `1px solid ${form.tier === t ? (t === 'premium' ? 'rgba(255,216,107,0.5)' : 'rgba(16,217,130,0.5)') : 'rgba(255,255,255,0.1)'}`,
              }}>
              <div className="text-[11px] font-bold uppercase tracking-wider" style={{ color: t === 'premium' ? '#ffd86b' : '#10D982' }}>{t}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-3">
        ...
        <div>
          <label className="block text-[10px] uppercase tracking-wider font-bold text-[#10D982] mb-1.5">Jurisdiction *</label>
          <select value={form.jurisdiction} onChange={update('jurisdiction')}
            className="w-full bg-black/40 outline-none text-white text-[12px] px-3 py-2 rounded border border-[rgba(16,217,130,0.2)] focus:border-[rgba(16,217,130,0.6)]">
            {JURISDICTIONS.map(j => <option key={j} value={j}>{j}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-[10px] uppercase tracking-wider font-bold text-[#10D982] mb-1.5">Category *</label>
      ...
      <div>
        <label className="block text-[10px] uppercase tracking-wider font-bold text-[#10D982] mb-1.5">Why are you applying? (optional)</label>
        <textarea value={form.motivation} onChange={update('motivation')} rows={4}
          ...
          className="w-full bg-black/40 outline-none text-white text-[12px] px-3 py-2 rounded border border-[rgba(16,217,130,0.2)] focus:border-[rgba(16,217,130,0.6)] resize-y" />
      </div>
      ...
      <button onClick={submit} disabled={loading}
        ...
        style={{ background: 'rgba(16,217,130,0.15)', border: '1px solid rgba(16,217,130,0.6)', color: '#10D982', boxShadow: '0 0 20px rgba(16,217,130,0.15)' }}>
...
function Field({ label, value, onChange, type, mono, placeholder }: { label: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; type?: string; mono?: boolean; placeholder?: string }) {
  return (
    <div>
      <label className="block text-[10px] uppercase tracking-wider font-bold text-[#10D982] mb-1.5">{label}</label>
      <input type={type ?? 'text'} value={value} onChange={onChange} placeholder={placeholder}
        className={`w-full bg-black/40 outline-none text-white text-[12px] px-3 py-2 rounded border border-[rgba(16,217,130,0.2)] focus:border-[rgba(16,217,130,0.6)] ${mono ? 'font-mono' : ''}`} />
    </div>
  )
}
