...
export default function TrialPage() {
  ...
  if (status === 'done') return (
    <div className="min-h-screen bg-[#050508] text-[#10D982] font-mono flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center space-y-6">
        <div className="w-16 h-16 rounded-full bg-[rgba(16,217,130,0.1)] border border-[rgba(16,217,130,0.4)] flex items-center justify-center mx-auto">
          <CheckCircle className="w-8 h-8 text-[#10D982]" />
        </div>
        ...
        <div className="rounded p-4 text-left space-y-2" style={{ background: 'rgba(16,217,130,0.04)', border: '1px solid rgba(16,217,130,0.15)' }}>
          {['30-minute live demo of your specific use case','Free 90-day pilot — no commitment','Direct access to the founding team','Custom gap report for your fund'].map(item => (
            <div key={item} className="flex items-center gap-2 text-[10px] text-[rgba(255,255,255,0.6)]"><span className="text-[#10D982]"></span> {item}</div>
          ))}
        </div>
        <a href="/scan" className="inline-flex items-center gap-2 px-6 py-3 rounded font-bold text-sm uppercase tracking-wider" style={{ background: 'rgba(16,217,130,0.1)', border: '1px solid rgba(16,217,130,0.4)', color: '#10D982' }}>
          <Zap className="w-4 h-4" /> Back to the scanner
        </a>
      </div>
    </div>
  )
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-3">
              {[
                ...
                { icon: AlertTriangle, title: 'Sanctions Screening', desc: 'Screening against the live OFAC SDN + EU consolidated lists' },
              ].map(({ icon: Icon, title, desc }) => (
                <div key={title} className="flex items-start gap-3 p-3 rounded" style={{ background: 'rgba(16,217,130,0.03)', border: '1px solid rgba(16,217,130,0.1)' }}>
                  <div className="w-7 h-7 rounded flex items-center justify-center shrink-0 mt-0.5" style={{ background: 'rgba(16,217,130,0.1)', border: '1px solid rgba(16,217,130,0.2)' }}>
                    <Icon className="w-3.5 h-3.5 text-[#10D982]" />
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-wider text-[#10D982]">{title}</div>
                    <div className="text-[9px] text-[rgba(255,255,255,0.4)] mt-0.5 leading-relaxed">{desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="p-4 rounded space-y-2" style={{ background: 'rgba(242,86,110,0.05)', border: '1px solid rgba(242,86,110,0.2)' }}>
              <div className="text-[9px] uppercase tracking-widest text-[#F2566E] font-bold">DORA deadline</div>
              <div className="text-2xl font-bold text-[#F2566E]">Jan 17, 2027</div>
          ...
          <div className="lg:col-span-3">
            <form onSubmit={submit} className="space-y-5 p-6 rounded" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.15)' }}>
              <div className="space-y-1 mb-6">
                ...
                <p className="text-[9px] text-[rgba(16,217,130,0.65)] tracking-wider mt-1.5 normal-case">🔒 The scanner runs in your browser — your fund data is never uploaded, and there is no LLM anywhere in the loop. A pilot is an evaluation, not a contract.</p>
              </div>
              ...
              {status === 'error' && <div className="text-[9px] text-[#F2566E] p-3 rounded" style={{ background: 'rgba(242,86,110,0.08)', border: '1px solid rgba(242,86,110,0.3)' }}>{errorMsg}</div>}
