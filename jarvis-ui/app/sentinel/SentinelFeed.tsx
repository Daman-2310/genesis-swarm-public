...
export default function SentinelFeed() {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl p-12 flex items-center justify-center"
        style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(91,141,239,0.2)' }}>
  ...
  return (
    <div className="rounded-2xl overflow-hidden"
      style={{ background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(91,141,239,0.2)', backdropFilter: 'blur(10px)' }}>
      <div className="px-4 py-2 flex items-center justify-between"
        style={{ background: 'rgba(91,141,239,0.06)', borderBottom: '1px solid rgba(91,141,239,0.15)' }}>
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-[#10D982]" style={{ animation: 'pulse 1s ease-in-out infinite' }} />
          <span className="text-[10px] uppercase tracking-wider font-bold text-[#10D982]">LIVE</span>
        </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {p.source_url && (
                <a href={p.source_url} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 mt-1 text-[10px] font-mono text-[#5B8DEF] hover:underline">
                  source ↗
                </a>
              )}
            </div>
