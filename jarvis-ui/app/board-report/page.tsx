...
function StatusDot({ s }: { s: 'green'|'amber'|'red' }) {
  const c = s === 'green' ? '#10D982' : s === 'amber' ? '#F5A524' : '#F2566E'
...
export default function BoardReportPage() {
  ...
  const oc = (s: 'green'|'amber'|'red') => s === 'green' ? '#10D982' : s === 'amber' ? '#F5A524' : '#F2566E'
    ...
    <div className="min-h-screen bg-[#050508] text-[#10D982] font-mono overflow-x-hidden">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b border-[rgba(16,217,130,0.12)]" style={{ background: 'rgba(5,5,8,0.97)', backdropFilter: 'blur(8px)' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[rgba(16,217,130,0.4)] text-[10px] tracking-widest hidden sm:block">// AI Board Report Generator</span>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8 print:py-4">
        {!report && (
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.5)]">Deterministic · SHA-256 sealed</div>
            ...
            <div className="max-w-xl space-y-4 p-6 rounded" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.15)' }}>
        ...
        {report && (
          <div className="space-y-6 print:space-y-4">
            {/* Cover */}
            <div className="p-6 rounded space-y-3" style={{ background: 'rgba(16,217,130,0.03)', border: '1px solid rgba(16,217,130,0.2)' }}>
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-[9px] uppercase tracking-[0.3em] text-[rgba(16,217,130,0.5)] mb-1">Quarterly Compliance Board Pack</div>
              ...
              <div className="text-[rgba(255,255,255,0.6)] text-sm leading-relaxed border-t border-[rgba(16,217,130,0.1)] pt-3">{report.overall_summary}</div>
            </div>

            {/* Sections */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {report.sections.map(s => (
                <div key={s.title} className="p-4 rounded space-y-3" style={{ background: 'rgba(16,217,130,0.02)', border: `1px solid ${s.status === 'green' ? 'rgba(16,217,130,0.2)' : s.status === 'amber' ? 'rgba(245,165,36,0.25)' : 'rgba(242,86,110,0.25)'}` }}>
                  <div className="flex items-center gap-2">
            ...
            {/* Priorities */}
            <div className="p-5 rounded space-y-3" style={{ background: 'rgba(245,165,36,0.04)', border: '1px solid rgba(245,165,36,0.2)' }}>
              <div className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-[#F5A524]" />
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#F5A524]">Board Priorities — Next Quarter</span>
              </div>
              <div className="space-y-2">
                {report.priorities.map((p, i) => (
                  <div key={i} className="flex items-start gap-3 text-[9px] text-[rgba(255,255,255,0.6)]">
                    <span className="w-5 h-5 rounded-full bg-[rgba(245,165,36,0.15)] border border-[rgba(245,165,36,0.3)] flex items-center justify-center shrink-0 text-[8px] font-bold text-[#F5A524]">{i+1}</span>
                    {p}
                  </div>
                ))}
              </div>
            ...
            {/* Footer */}
            <div className="flex items-center justify-between p-4 rounded text-[8px] text-[rgba(255,255,255,0.25)]" style={{ background: 'rgba(16,217,130,0.02)', border: '1px solid rgba(16,217,130,0.08)' }}>
              <div>Generated by Genesis Swarm · deterministic engine · SHA-256 sealed · {report.report_id}</div>
              <div className="font-mono text-[7px] truncate max-w-[200px]">{report.certification_hash.substring(0,32)}…</div>
            </div>
            <div className="flex gap-3 print:hidden">
              <button onClick={() => setReport(null)} className="px-5 py-2.5 rounded text-sm font-bold uppercase tracking-wider transition-all" style={{ background: 'rgba(16,217,130,0.06)', border: '1px solid rgba(16,217,130,0.3)', color: '#10D982' }}>← New report</button>
