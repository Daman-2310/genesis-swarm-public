...
const URGENCY_COLOR: Record<string, string> = {
  CRITICAL: '#F2566E', HIGH: '#ff8800', MEDIUM: '#F5A524', LOW: '#10D982',
}
    ...
    <div className="min-h-screen bg-[#050508] text-[#10D982] font-mono select-none">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-6 py-3 border-b border-[rgba(16,217,130,0.12)]"
        ...
        <div className="flex items-center gap-3">
          <Radio className="w-4 h-4 text-[#F2566E] animate-pulse" />
        ...
        <div className="flex items-center gap-2">
          {['AIF', 'UCITS', 'RAIF', 'ALL'].map(t => (
            <button key={t} onClick={() => setFundType(t)}
              ...
              style={{ background: fundType === t ? 'rgba(16,217,130,0.15)' : 'transparent',
                border: `1px solid ${fundType === t ? 'rgba(16,217,130,0.5)' : 'rgba(255,255,255,0.1)'}`,
                color: fundType === t ? '#10D982' : 'rgba(255,255,255,0.4)' }}>
              {t}
            </button>
          ))}
          <a href="/dashboard" className="text-[9px] uppercase tracking-wider px-3 py-1 border border-[rgba(16,217,130,0.3)] text-[#10D982] rounded ml-2">← Back</a>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        {data && (
          <div className="grid grid-cols-4 gap-3">
            {[
              { label: 'Total Alerts', value: data.total_alerts, color: '#10D982' },
              { label: 'Critical', value: data.critical_count, color: '#F2566E' },
              { label: 'High Priority', value: data.high_count, color: '#ff8800' },
              { label: 'Next Deadline', value: data.next_deadline, color: '#F5A524' },
        ...
        {/* Upload circular */}
        <div className="p-4 rounded" style={{ background: 'rgba(91,141,239,0.04)', border: '1px solid rgba(91,141,239,0.15)' }}>
          ...
          <label className="flex items-center gap-2 px-4 py-2 rounded text-[10px] uppercase tracking-wider cursor-pointer transition-all w-fit"
            style={{ background: uploading ? 'rgba(91,141,239,0.04)' : 'rgba(91,141,239,0.08)', border: '1px solid rgba(91,141,239,0.35)', color: '#5B8DEF' }}>
            <Upload className="w-3 h-3" />
            ...
            <div className="mt-3 p-3 rounded space-y-2" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              {circularResult.action_items?.map((a: string, i: number) => (
                <div key={i} className="flex items-start gap-2 text-[8px] text-[rgba(255,255,255,0.5)]">
                  <ChevronRight className="w-2.5 h-2.5 text-[#10D982] mt-0.5 shrink-0" /><span>{a}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Alert list */}
        {loading ? (
          <div className="text-[9px] text-[rgba(16,217,130,0.5)] animate-pulse text-center py-8">Scanning CSSF + ESMA feeds…</div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-[8px] text-[rgba(255,255,255,0.3)]">
                        ...
                        <span className="text-[#F5A524]">Deadline: {alert.deadline}</span>
                      </div>
                    </div>
                  </div>
                ...
                {expanded === alert.id && (
                  <div className="px-4 pb-4 space-y-3 border-t border-[rgba(255,255,255,0.05)]">
                    ...
                    <div className="space-y-1.5">
                      <div className="text-[8px] uppercase tracking-widest text-[rgba(16,217,130,0.5)]">Action Required</div>
