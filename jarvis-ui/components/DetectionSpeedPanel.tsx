  ...
  return (
    <div className="terminal-border bg-genesis-surface flex flex-col h-full">
      ...
      <div
        ...
        style={{ borderBottom: '1px solid rgba(16,217,130,0.1)' }}
      >
        <div>
          <div style={{ fontSize: '9px', fontWeight: 700, color: '#10D982', letterSpacing: '0.1em' }}>
            DETECTION BENCHMARK
          </div>
          ...
          {mode === 'crisis' ? (
            <button
              ...
              style={{
                ...
                background: 'rgba(16,217,130,0.1)', border: '1px solid rgba(16,217,130,0.3)',
                color: '#10D982', cursor: 'pointer', letterSpacing: '0.06em',
              }}
            >
              RESET
            </button>
            ...
            <button
              ...
              style={{
                fontSize: '9px', padding: '4px 10px', fontFamily: 'JetBrains Mono, monospace',
                background: loading ? 'rgba(242,86,110,0.05)' : 'rgba(242,86,110,0.15)',
                border: '1px solid rgba(242,86,110,0.5)',
                color: '#F2566E', cursor: loading ? 'not-allowed' : 'pointer', letterSpacing: '0.06em',
              }}
            >
      ...
      <div className="flex-1 p-3 flex flex-col gap-3 overflow-auto" style={{ minHeight: 0 }}>
        ...
        {mode === 'crisis' && demo && (
          <div
            style={{
              background: 'rgba(242,86,110,0.1)',
              border: '1px solid rgba(242,86,110,0.4)',
              padding: '8px 12px',
            }}
          >
            <div style={{ fontSize: '10px', fontWeight: 700, color: '#F2566E', marginBottom: '4px', letterSpacing: '0.08em' }}>
              WIRECARD ANALOG CRISIS DETECTED
            </div>
            <div className="flex gap-4 flex-wrap">
              <div>
                <div style={{ fontSize: '8px', color: 'rgba(242,86,110,0.6)' }}>DETECTION TIME</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: '#F2566E' }}>
                  {demo.detection_time_ms}ms
                </div>
              </div>
              <div>
                <div style={{ fontSize: '8px', color: 'rgba(242,86,110,0.6)' }}>CAPITAL AT RISK</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: '#F2566E' }}>
                  €{(demo.total_at_risk_eur_m / 1000).toFixed(1)}B
                </div>
              </div>
              <div>
                <div style={{ fontSize: '8px', color: 'rgba(16,217,130,0.5)' }}>TRADITIONAL WOULD CATCH IN</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: 'rgba(16,217,130,0.4)' }}>
            ...
            <div className="mt-2 flex flex-col gap-1">
              {demo.timeline.map((step, i) => (
                <div key={i} className="flex gap-2" style={{ fontSize: '9px' }}>
                  <span style={{ color: 'rgba(242,86,110,0.5)', minWidth: '60px' }}>{step.t}</span>
                  <span style={{ color: 'rgba(16,217,130,0.7)' }}>{step.event}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Comparison table */}
        <div>
          <div style={{ fontSize: '8px', color: 'rgba(16,217,130,0.4)', letterSpacing: '0.08em', marginBottom: '6px' }}>
            HEAD-TO-HEAD COMPARISON:
          </div>
          <div style={{ border: '1px solid rgba(16,217,130,0.1)' }}>
            ...
            {COMPARISON_ROWS.map((row, i) => (
              <div
                ...
                style={{ borderBottom: i < COMPARISON_ROWS.length - 1 ? '1px solid rgba(16,217,130,0.06)' : 'none', fontSize: '9px' }}
              >
                <span style={{ color: 'rgba(16,217,130,0.5)' }}>{row.label}</span>
                <span style={{ color: '#F5A524' }}>{row.traditional}</span>
                <span style={{ color: '#10D982', fontWeight: 700 }}>{row.genesis}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {WIRECARD_TIMELINE.map((item, i) => (
              <div key={i} className="flex gap-3 items-start" style={{ fontSize: '9px' }}>
                <span style={{ color: item.caught ? '#10D982' : 'rgba(245,165,36,0.6)', minWidth: '72px', fontWeight: 700 }}>
                  {item.year}
                </span>
                <span style={{ color: item.caught ? '#10D982' : 'rgba(16,217,130,0.4)' }}>
          ...
          <div
            style={{
              ...
              background: 'rgba(16,217,130,0.04)', border: '1px solid rgba(16,217,130,0.15)',
