...
CONSTANTS
    ...
    MAX_SCORE           \* Upper bound on telemetry anomaly score (bounds the
                        \* UpdateTelemetry branching so TLC is tractable)
...
\* (Removed unused helpers MaxOver/SumOver — SumOver referenced SetToSeq, which
\*  is a CommunityModules operator not imported here; both were dead code and
\*  caused a semantic error under SANY/TLC.)
...
HasConcentrationBreach(id) == agentBreaches[id] \div 4 % 2 = 1
...
NextHash(h, entry) == (h * 31 + entry.agent_id * 1000003 + entry.round) % (2^32)
...
SendPrePrepare(i) ==
    ...
    /\ i = agentRound[i] % N_AGENTS          \* this agent is the current leader
...
ViewChange(i) ==
    ...
    /\ LET leader == agentRound[i] % N_AGENTS
...
Next ==
    ...
    \/ \E i \in HonestAgents, g \in 0..AIFMD_GROSS_LIMIT,
               ...
               s \in 0..MAX_SCORE : UpdateTelemetry(i, g, n, s)
