...
**Real-Time AIFMD II · DORA · MiFID II Compliance — nanosecond hot-path checks · ~2 ms real ZK verification**
...
[![Latency](https://img.shields.io/badge/Hot%20Path-292%20ns%20(excl.%20ZK)-00C851?style=flat-square)](sovereign-engine/target/criterion/end_to_end/)
[![BFT](https://img.shields.io/badge/Byzantine%20Tolerance-F%20%3D%203%20of%2011-FF6B35?style=flat-square)](/)

---

> *"An open-source, Byzantine-resilient compliance engine with a TLA+ specification*
> *(TLC model-checking in progress), purpose-built for Luxembourg fund compliance."*

---

</div>

## Engineering honesty — what is production-grade vs reference implementation

This project is built and maintained solo. Rather than let you discover the line
between the two, here it is, stated plainly. Everything below is checkable in this repo.

**Genuinely real and production-grade today**
- **Deterministic compliance engine** — the Rust circuit-breaker (AIFMD leverage / DORA
  thresholds) and the TypeScript prospectus scanner (`jarvis-ui/lib/scan-engine.ts`) are
  pure, branch-exact logic with unit tests. No LLM decides a verdict, so results are
  reproducible and auditable.
- **Real cryptography** — BN254 Groth16 verification via `arkworks`
  (`verify_compliance_proof` in `sovereign-engine/src/main.rs` runs the real pairing
  check); SHA-256 hash-chained ledger; real Ed25519 signing of audit artifacts
  (`jarvis-ui/lib/signing.ts`, served at `/api/sign`, verifiable with standard Ed25519).
- **Live, real external data** — GLEIF LEI registry lookups (`/api/lei`).
- **Per-tenant persistence** — Supabase Postgres with row-level security
  (`/api/vault`), isolation enforced at the database.

**Reference implementation / simulation (NOT yet production)**
- **Consensus is single-node.** The 11-agent PBFT loop runs in-process on one machine —
  it demonstrates the protocol, it is not a deployed geo-distributed Byzantine cluster.
- **Benchmark headline (≈292 ns)** is the hot path (circuit-breaker + quorum + SHA-256
  ledger) measured with the **stub-ZK sentinel** (zeroed proof bytes). Real Groth16
  verification is ≈2 ms — the benchmark file labels the stub path as `stub`.
- **TLA+ spec: safety invariants now TLC-verified on a scaled finite instance.**
  Two real defects were fixed first (an invalid `\mod` operator that stopped the
  spec from parsing, and an undefined helper that broke semantic analysis — so it
  had *never actually run* before). With those fixed, TLC exhaustively explored
  **all 32,825,344 reachable states** of the scaled model (N=4, f=1, depth 16;
  see `sovereign-engine/genesis_mesh.cfg`) with **no counterexample** to any of the
  four safety invariants or the type invariant. **Not** machine-checked: the two
  temporal/liveness properties, and the full N=11 instance.
- **The hosted web demo** is a Next.js frontend (Vercel) calling a free-tier HuggingFace
  Space backend — a demo, not infrastructure handling real institutional AUM.

If a number or claim anywhere else in this README reads stronger than this section,
**this section is the source of truth.**

## Try it live — in your browser, nothing uploaded

The deterministic engine runs entirely client-side: no document ever leaves your
device, and every verdict is reproducible and hash-sealed.

| Surface | What it does |
|---|---|
| [/scan](https://genesis-swarm-rgq5.vercel.app/scan) | Paste a prospectus → an AIFMD II verdict in ~300 ms, sealed into a SHA-256 hash anyone can re-verify |
| [/shadow](https://genesis-swarm-rgq5.vercel.app/shadow) | **Shadow Mode** — run alongside an existing process; per-document divergence report + board-ready, hash-chained audit pack + deterministic reasoning trace + resilience scorecard |
| [/deterministic](https://genesis-swarm-rgq5.vercel.app/deterministic) | Why deterministic beats black-box AI for compliance — with a **live** throughput benchmark and a determinism proof (same input → identical hash) you run yourself |
| [/vault](https://genesis-swarm-rgq5.vercel.app/vault) | Per-tenant, row-level-security-isolated evidence ledger |

Source: [`jarvis-ui/lib/`](jarvis-ui/lib/) — `scan-engine.ts`, `shadow.ts`,
`audit-pack.ts`, `reasoning-trace.ts`, `perf-benchmark.ts`, each with unit tests
in [`jarvis-ui/tests/`](jarvis-ui/tests/).
...
```
 ...
 │  TLC-checked (safety): 32.8M states, no error                           │
 └─────────────────────────────────────────────────────────────────────────┘
...
TLC exhaustively explores all reachable states of a scaled instance
(`N_AGENTS=4, BYZANTINE_COUNT=1, MAX_ROUND=2, MAX_QUEUE=2`; magnitudes reduced for
tractability — see `genesis_mesh.cfg`). Verified: 32,825,344 distinct states,
depth 16, no counterexample. The full N=11 instance and the liveness properties
...
```bash
# Run the TLC model checker (TLA+ tools: tla2tools.jar)
cd sovereign-engine
java -cp tla2tools.jar tlc2.TLC -workers auto -config genesis_mesh.cfg genesis_mesh.tla
# Verified result: "Model checking completed. No error has been found."
#   32,825,344 distinct states · depth 16 · 0 left on queue (exhaustive)
# Scope: the 5 safety invariants on the scaled instance. The 2 temporal
# (liveness) properties are specified in the .tla but NOT checked in this run.
```
